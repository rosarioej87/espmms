...
(function (window, $, undefined) {
  ...
  Grid.prototype = {
    ...
    _initController: function () {
      this._controller = $.extend(
        {},
        defaultController,
        getOrApply(this.controller, this)
      );
    },
      ...
      self.fields = $.map(self.fields, function (field) {
        if ($.isPlainObject(field)) {
          var fieldConstructor =
            (field.type && jsGrid.fields[field.type]) || jsGrid.Field;
    ...
    option: function (key, value) {
      ...
      this._handleOptionChange(
        optionChangingEventArgs.option,
        optionChangingEventArgs.newValue
      );
    ...
    _createHeader: function () {
      ...
      var $headerGrid = (this._headerGrid = $("<table>")
        .addClass(this.tableClass)
        .append($headerRow)
        .append($filterRow)
        .append($insertRow));

      var $header = (this._header = $("<div>")
    ...
    _createBody: function () {
      ...
      var $bodyGrid = (this._bodyGrid = $("<table>")
        .addClass(this.tableClass)
        .append($content));

      var $body = (this._body = $("<div>")
    ...
    _createPagerContainer: function () {
      var pagerContainer =
        this.pagerContainer || $("<div>").appendTo(this._container);
    ...
    _createHeaderRow: function () {
      ...
      this._eachField(function (field, index) {
        ...
        if (this.sorting && field.sorting) {
          $th.addClass(this.sortableClass).on(
            "click",
            $.proxy(function () {
              this.sort(index);
            }, this)
    ...
    _createRow: function (item, itemIndex) {
      ...
      $result
        .addClass(this._getRowClasses(item, itemIndex))
    ...
    _createCell: function (item, field) {
      ...
      if ($.isFunction(field.cellRenderer)) {
        ...
        $result = $("<td>").append(
          this.renderTemplate(field.itemTemplate || fieldValue, field, args)
        );
      }
    ...
    _sortData: function () {
      ...
      if (sortField) {
        this.data.sort(function (item1, item2) {
          return (
            sortFactor *
            sortField.sortingFunc(item1[sortField.name], item2[sortField.name])
          );
        });
      }
    },
    ...
    _pagesCount: function () {
      ...
      return (
        Math.floor(itemsCount / pageSize) + (itemsCount % pageSize ? 1 : 0)
      );
    },
    ...
    _createPagerByFormat: function () {
      ...
      return $.map(
        pagerParts,
        $.proxy(function (pagerPart) {
          var result = pagerPart;

          if (pagerPart === PAGES_PLACEHOLDER) {
            result = this._createPages();
          } else if (pagerPart === FIRST_PAGE_PLACEHOLDER) {
            result = this._createPagerNavButton(
              this.pageFirstText,
              1,
              pageIndex > 1
            );
    ...
    _createPages: function () {
      ...
      for (
        var i = 0, pageNumber = firstDisplayingPage;
        i < pageButtonCount && pageNumber <= pageCount;
        i++, pageNumber++
      ) {
        pages.push(
    ...
    _createPagerNavButton: function (text, pageIndex, isActive) {
      return this._createPagerButton(
        ...
        this.pagerNavButtonClass +
          (isActive ? "" : " " + this.pagerNavButtonInactiveClass),
        isActive
          ? function () {
    ...
    showNextPages: function () {
      ...
      this._firstDisplayingPage =
        firstDisplayingPage + 2 * pageButtonCount > pageCount
    ...
    _validateItem: function (item, $row) {
      ...
      this._eachField(function (field) {
        ...
        var errors = this._validation.validate(
          $.extend(
            {
              value: fieldValue,
        ...
        this._setCellValidity(
          $row.children().eq(this._visibleFieldIndex(field)),
          errors
        );

        if (!errors.length) return;

        validationErrors.push.apply(
          validationErrors,
    ...
    _createEditRow: function (item) {
      ...
      this._eachField(function (field) {
        ...
        this._prepareCell("<td>", field, "editcss")
          .append(
            this.renderTemplate(field.editTemplate || "", field, {
              value: fieldValue,
              item: item,
            })
          )
    ...
    _updateRow: function ($updatingRow, editedItem) {
      ...
      return this._controllerCall(
        ...
        updatedItem,
        args.cancel,
        function (loadedUpdatedItem) {
          var previousItem = $.extend(true, {}, updatingItem);
          updatedItem =
            loadedUpdatedItem || $.extend(true, updatingItem, editedItem);

          var $updatedRow = this._finishUpdate(
            $updatingRow,
            updatedItem,
            updatingItemIndex
          );
    ...
    _getEditRow: function () {
      return (
        this._editingRow && this._editingRow.data(JSGRID_EDIT_ROW_DATA_KEY)
      );
    },
    ...
    deleteItem: function (item) {
      ...
      if (
        ...
        !window.confirm(
          getOrApply(this.deleteConfirm, this, $row.data(JSGRID_ROW_DATA_KEY))
        )
      )
        return;
  ...
  var setLocale = function (obj, localeConfig) {
    $.each(localeConfig, function (field, value) {
      if ($.isPlainObject(value)) {
        setLocale(
          obj[field] || obj[field[0].toUpperCase() + field.slice(1)],
          value
        );
        return;
      }
  ...
  DirectLoadingStrategy.prototype = {
    firstDisplayIndex: function () {
      ...
      return grid.option("paging")
        ? (grid.option("pageIndex") - 1) * grid.option("pageSize")
        : 0;
    },
    ...
    lastDisplayIndex: function () {
      ...
      return grid.option("paging")
        ? Math.min(
            grid.option("pageIndex") * grid.option("pageSize"),
            itemsCount
          )
        : itemsCount;
    },
  ...
  Validation.prototype = {
    ...
    validate: function (args) {
      ...
      $.each(this._normalizeRules(args.rules), function (_, rule) {
        ...
        var errorMessage = $.isFunction(rule.message)
          ? rule.message(args.value, args.item)
          : rule.message;
    ...
    _normalizeRule: function (rule) {
      if (typeof rule === "string") rule = { validator: rule };

      if ($.isFunction(rule)) rule = { validator: rule };

      if ($.isPlainObject(rule)) rule = $.extend({}, rule);
      else throw Error("wrong validation config specified");
...
(function (jsGrid, $, undefined) {
  ...
  function SelectField(config) {
    ...
    if (config.valueField && config.items.length) {
      ...
      this.valueType =
        typeof firstItemValue === numberValueType
          ? numberValueType
          : stringValueType;
    }
  ...
  SelectField.prototype = new NumberField({
    ...
    itemTemplate: function (value) {
      ...
      if (valueField) {
        resultItem =
          $.grep(items, function (item, index) {
  ...
  ControlField.prototype = new Field({
    ...
    filterTemplate: function () {
      ...
      return this.clearFilterButton
        ? $result.add(this._createClearFilterButton())
        : $result;
    },
    ...
    _createOnOffSwitchButton: function (option, cssClass, isOnInitially) {
      ...
      var $button = this._createGridButton(
        this.modeButtonClass + " " + cssClass,
        "",
        function (grid) {
          isOn = !isOn;
      ...
      var updateButtonState = $.proxy(function () {
        $button
          .attr(
            "title",
            isInserting
              ? this.searchModeButtonTooltip
              : this.insertModeButtonTooltip
          )
