...
$(function() {
  ...
  /* Tasks interactions */
  initOnClickRestore    ( ".frontend .frontend-heading .toolbar .restore" );
  initOnClickSave       ( ".frontend .frontend-heading .toolbar .save" );
  initOnClickToggle     ( ".frontend .frontend-heading .toolbar .toggle" );
  initOnClickDelete     ( ".frontend .frontend-heading .toolbar .delete" );
  initOnclickColor      ( ".frontend .frontend-body .palette .color" );

  initDraggable         ( ".frontend" );
});
...
function initDom() {
  ...
  for (var i = 0; i < data.tasks.length; i++) {
    ...
    if( taskJson.g_context.active) {
      ...
      $('#'+taskJson._id).children(".frontend-heading").children(".toolbar").children(".restore").css("visibility", "visible");
      inactive++;
    }
  }
...
function dom2json (selector, options)
{
  ...
  if (options.indexOf("title") != -1) {
    json += ", \"title\": \""+task.children(".frontend-title").text() + "\"";
  }
  if (options.indexOf("text") != -1) {
    json += ", \"text\": \""+task.children('.frontend-body').children('.text').val() + "\"";
  }
  if (options.indexOf("geometry") != -1 || options.indexOf("col") != -1 || options.indexOf("colors") != -1) {
    ...
    if (options.indexOf("colors") != -1) {
      if(comma) { json += ", "; } else { comma = true; }
      json += "\"colors\": { ";
        json += "\"color1\": \"" + task.children('.frontend-heading').css("backgroundColor") + "\", ";
        json += "\"color2\": \"" + task.children('.frontend-body').css("backgroundColor") + "\", ";
        json += "\"color3\": \"" + task.children('.frontend-heading').css("color") + "\"";
      json += " }";
    }
    json += " }";
  }
...
function json2dom (taskJson) {
  ...
  var card = '';
  card += '<div id="'+taskJson._id+'" class="frontend frontend-default ui-draggable ui-draggable-handle card lighten-3';
  ...
  style += taskJson.g_context.visible ? "" : "; height: "+data.g_context.task.headHeight+"px";
    ...
    card += '<div class="frontend-heading card-action lighten-2" style="'+style+'">';
      card += '<div class="frontend-title">'+taskJson.title+'</div>';
    ...
    card += '<div class="frontend-body card-content" style="'+style+'">';
...
function centerPosition(selector) {
  //return { "top": g_context_default.frontend.top, "left": g_context_default.task.left };
...
function initOnClickNew (selector) {
  $( selector ).click(function() {
    var n = $(".col:first > .frontend").length + 1;
...
function initOnclickColor (selector) {
  $( selector ).click(function() {
    ...
    task.children('.frontend-heading').css({backgroundColor: color1, color: color3});
  ...
  $( selector ).draggable({
      ...
      stop: function(event, ui) {
        ...
        if(taskJson.g_context.active)
        {
          ...
          tasks = $( "#"+taskJson.g_context.col).children(".frontend");
        }
        else {
          $( "#trash #"+taskJson.g_context.col).append($('#' + id));
          tasks = $( "#trash #"+taskJson.g_context.col).children(".frontend");
        }
        log(tasks);
...
socket.on('socket_update', function(taskJson) {
  ...
  if (taskJson.text != null) {
    task.children(".frontend-body").children(".text").val(taskJson.text)
  }
  if (taskJson.g_context != null && taskJson.g_context.colors != null) {
    task.children('.frontend-heading').css({backgroundColor: taskJson.g_context.colors.color1, color: taskJson.g_context.colors.color3});
    task.children(".frontend-body").css({backgroundColor: taskJson.g_context.colors.color2});
    task.children(".frontend-body").children('.text').css({backgroundColor: taskJson.g_context.colors.color2, color: taskJson.g_context.colors.color3})
  }
  if(taskJson.g_context.active) {
    $( "#"+taskJson.g_context.col).append(task);
  }
...
socket.on('socket_restore', function(id) {
  ...
  task.children(".frontend-heading").children(".toolbar").children(".restore").css("visibility", "hidden");
...
socket.on('socket_delete', function(id) {
  ...
  if(task.hasClass("active"))
  {
    ...
    task.children(".frontend-heading").children(".toolbar").children(".restore").css("visibility", "visible");
    var colId = task.parent().attr('id');
...
socket.on('socket_toggle', function(json) {
  ...
  if(json.visible == true) {
    log("show");
    $( '#' + json.id ).children(".frontend-body").show(300);
    $( '#' + json.id ).height(json.height);
  }
  else {
    log("hide");
    log($( '#' + json.id ));
    var headHeight = $( '#' + json.id ).children(".frontend-heading").height();
    $( '#' + json.id ).children(".frontend-body").hide(300);
    $( '#' + json.id ).height(data.g_context.task.hiddenHeight);
    $( '#' + json.id ).children(".frontend-heading").height(headHeight);
  }
});
...
socket.on('socket_new', function(taskJson) {
  ...
  initOnClickRestore  ( "#" + taskJson._id + " .frontend-heading .toolbar .restore" );
  initOnClickSave     ( "#" + taskJson._id + " .frontend-heading .toolbar .save"    );
  initOnClickToggle   ( "#" + taskJson._id + " .frontend-heading .toolbar .toggle"  );
  initOnClickDelete   ( "#" + taskJson._id + " .frontend-heading .toolbar .delete"  );
  initOnclickColor    ( "#" + taskJson._id + " .frontend-body .palette .color"      );
